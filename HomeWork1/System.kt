...
/*
Как добавить пуш:
    1. Если в вашем пуше есть новые поля => добавте его в Push.companion (Просто что бы следовать внутреннему кодстайлу)
    2. Создайте класс нового пуша наследуясь от Push, естественно реализовав методы.
    3. Добавте его в pushTypes в PushTask - почему так? чтобы можно было легко добавлять и убирать пуши которые работают.

Комментарии:
1. Я не понял как сделать чтобы в интерфейсе или абстрактном классе сделать метод который необходимо реализовать в companion
Это для того, чтобы сделать так:
    val pushTypes: Map<String, Push> = mapOf("GenderPush" to GenderPush),
    pushTypes["GenderPush"]!!.newInstance(params)
Если вы знаете, пожалуйста напишите мне в тг @SysteamName

2. Я знаю что не очень хорошо пихать в Push информацию, которая ему не нужна, но так лучше расширяемость.

3. По хорошему еще и фильтры нужно разделить, но я не хочу сейчас этим заниматься.
 */
abstract class Push {
